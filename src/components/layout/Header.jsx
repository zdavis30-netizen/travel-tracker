...
export function Header({ start, end, activePreset, onPreset, onCustomRange, onAddEvent, onImport, activeView, onViewChange, isLive }) {
        ...
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
                <button
                  ...
                  className={`px-3 py-1.5 font-medium transition-colors cursor-pointer ${activeView === 'calendar' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                >
                  Calendar
                </button>
                <button
                  onClick={() => onViewChange('events')}
                  className={`px-3 py-1.5 font-medium transition-colors cursor-pointer ${activeView === 'events' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                >
                  All Events
                </button>
              </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button variant="ghost" size="sm" onClick={() => setSettingsOpen(true)} title="Flight API settings">
      ...
      <Modal isOpen={settingsOpen} onClose={() => setSettingsOpen(false)} title="Flight Lookup Settings">
        <div className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            {getApiKey() && (
              <Button variant="secondary" onClick={() => { saveApiKey(''); setApiKey(''); setSettingsOpen(false); }}>
                Remove Key
              </Button>
            )}
          </div>
